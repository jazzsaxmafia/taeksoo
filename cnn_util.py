...
def makeCropImage(x, target_height=227, target_width=227):
    print x
...
class CNNUtil(object):
    ...
    def __init__(self, deploy=deploy, model=model, mean=mean, batch_size=10, width=227, height=227):
        ...
        self.net, self.transformer = self.getNet()
    ...
    def imageFeatures(self, image_list, layers='fc7', layer_sizes=4096):
        ...
        for start, end in zip(range(0, iter_until, self.batch_size), \
            ...
            image_batch = np.array(map(lambda x: makeCropImage(x, target_width=227, target_height=227), image_batch_file))
